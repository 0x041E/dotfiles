...
beautiful.init(gears.filesystem.get_themes_dir() .. "green/theme.lua")
...
local musicwidget = awful.widget.button({ image = themedir .. "/actions/24/music-library.png" })
-- detect cmus on startup used particularly when restarting awesome
...
awful.rules.rules = {
    ...
    { rule_any = {name = { "KakaoTalk" }
      }, properties = { floating = true, titlebars_enabled = false }
    },
    { rule_any = { class = { "URxvt", "XTerm", "mpv" }
...
-- awful.spawn("run-once xss-lock -- i3lock-fancy")
-- awful.spawn("run-once i3lock-fancy -- scrot")
awful.spawn("run-once nm-applet")
-- awful.spawn("run-once connman-gtk")
-- awful.spawn("ibus-daemon -xrd")
